...
class Flake {
	constructor(p5Canvas) {
		...
		this.pos = new vec2(
			...
			Math.floor(Math.random() * this.canvas.height * snow.length) - (this.canvas.height * snow.length)
		);
		this.vel = new vec2(
	...
	update() {
		...
		if (this.pos.x > this.canvas.width+this.margin) this.pos.x = 0;
		if (this.pos.x < -this.margin) this.pos.x = this.canvas.width;
	...
	show() {
		this.canvas.strokeWeight(this.size);
...
var snow;
try {
	snow = new Array(100*navigator.deviceMemory);
} catch (error){
	snow = new Array(100);
}
...
function img_sim(p) {
	...
	p.draw = function () {
		p.background(0);
		for (let i = 0; i < snow.length; i++) { snow[i].update(); }
		for (let i = 0; i < snow.length; i++) { snow[i].show(); }
	}
}
